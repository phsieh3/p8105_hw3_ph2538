...
ggplot(five_panel_plot, aes(x = year, y = proportion, color = state)) + geom_line() + facet_grid(. ~ response)
...
popular_item = 
  instacart %>% 
  filter(aisle == "baking ingredients" | aisle == "dog food care" | aisle == "packaged vegetable fruits") %>% 
  group_by(aisle)
